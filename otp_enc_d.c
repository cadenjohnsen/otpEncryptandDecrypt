/*
* waits in background for opt_enc to be called and then
* receives plaintext and cipher over specified port
* encrypts the plaintext and returns it to opt_enc
* similar to server.c
*/
#include <stdio.h>
#include <stdlib.h>
...
char* encryptMessage(char plaintext[25000], char key[25000]) {
	int i, j, temp;
	...
	for(i = 0; i < strlen(plaintext); i++) {	// loop through plaintext string
		modPlainText[i] = plaintext[i];	// turn the character into a numerical value
		if(modPlainText[i] == 32) {	// if the character is a space move it into the last place on the index
			modPlainText[i] = 26;
		}
		else if(modPlainText[i] == 0) {	// if the character is NULL make it an impossible value
			modPlainText[i] = 49;
		}
		else if(modPlainText[i] > 64 && modPlainText[i] < 91) {	// if the character is a valid letter then bring it down to 27 modulo table
			modPlainText[i] = modPlainText[i] - 65;
		}
		else {
			fprintf(stderr, "ERROR plaintext file has invalid character: %c\n", plaintext[i]);	// print error for invalid character
		}
		...
		modCipherText[i] = modPlainText[i] + modKey[i];	// add the plaintext and key values together
		if(modCipherText[i] < 0) {	// if it is a negative number turn it back into a character
			modCipherText[i] += 27;
		}
		if(modCipherText[i] > 26) {	// if it is larger than the modulo subtract 27 to turn it into a character
			modCipherText[i] -= 27;
		}
		tempChar = modCipherText[i];	// assign the numerical value into a character
		if(modPlainText[i] < 27) {	// if the character is a space then set it to space
			if(tempChar == 26) {
				tempChar = 32;
			}
...
int main(int argc, char const *argv[]) {
  ...
  char key[25000];
	...
	ciphertext = malloc(25000 * sizeof(char));	// allocate memory for ciphertext
...
while(1) {
		...
		if (establishedConnectionFD < 0) error("ERROR on accept");
			...
			memset(plaintext, '\0', sizeof(plaintext)); // Clear out the ciphertext again for reuse
			...
			if (charsRead < 0) error("ERROR reading from socket");
				...
				size = strlen(buffer)/2;	// split the buffer in half for plaintext and key
				for(i = 0; i < size; i++) {
					plaintext[i] = buffer[i];	// first half of buffer goes into plaintext
					key[i] = buffer[i + size];	// last half of buffer goes into key
				}

				if(strlen(key) < strlen(plaintext)) {	// check if key is smaller than plaintext
					fprintf(stderr, "ERROR key is smaller than plaintext\n");
				}

				strcpy(ciphertext, encryptMessage(plaintext, key));	// call encrypt function and copy it into ciphertext
